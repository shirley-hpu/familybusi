...
import com.sitech.family.custservice.atombusi.IFamilyMbrAttrMidInfo;
...
public class FamilyMbrPrcMidCo implements IFamilyMbrPrcMidCo {
    ...
    public ReturnDto create(Map<String, Object> map) {
        ...
        try {
                            ...
                            if (StringUtil.isNotEmptyOrNull(familyUserPrcMbrIns.getPrcId())) {
                                ...
                                if (StringUtil.isNotEmptyOrNull(familyUserPrcMbrIns.getFamilyMbrPrcAttrMidList())) {
                                    ...
                                    for (FamilyMbrAdd.FamilyMbrPrcAttrMid familyMbrPrcAttrMid : familyMbrPrcAttrMidList) {
                                        ...
                                        userMbrAttrMidInfo.setInsType(InsTypeEnum.VAL_2.getValue());
                                        userMbrAttrMidInfo.setInsValue(new BigDecimal(prodPrcInsId));
                                        userMbrAttrMidInfo.setAttrId(familyMbrPrcAttrMid.getAttrId());
                                        if (StringUtil.isNotEmptyOrNull(familyUserPrcMbrIns.getEffDate())) {
                                            userMbrAttrMidInfo.setEffDate(DateUtil.toDate(familyUserPrcMbrIns.getEffDate(), DateConst.DATE_MODEL_8));
                                        }
            ...
            throw new BusiException(getErrorCode("1000", "15000"), "家庭在途订购实例创建失败" + e.getMessage());
        }
     ...
     */
    @Override
    public ReturnDto delByIdNo(Map<String, Object> map) {
        ReturnDto returnDto = new ReturnDto();
        ...
        try {
            ...
            for (UserGroupMbrInfo userGroupMbrInfo : userGroupMbrInfoList) {
                Long idNo = Long.valueOf(userGroupMbrInfo.getObjectId());

                Map<String, Object> attrMap = new HashMap<>();
                attrMap.put("idNo", idNo);
                List<UserMbrPrcMidInfo> prcMidList = iFamilyMbrPrcMidInfo.qryUserMbrPrcMidInfoList(attrMap);

                returnDto = iFamilyMbrPrcMidInfo.delByIdNo(updateOprInfo, idNo);
                ...
                for (UserMbrPrcMidInfo userMbrPrcMidInfo : prcMidList) {
                    if (InsTypeEnum.VAL_1.getValue().equals(userMbrPrcMidInfo.getInsType()) || InsTypeEnum.VAL_3.getValue().equals(userMbrPrcMidInfo.getInsType())) {
                        BigDecimal insValue = userMbrPrcMidInfo.getInsValue();
                        returnDto = iFamilyMbrAttrMidInfo.delByInsValue(updateOprInfo, insValue);
                    }
                }
            }
        } catch (Exception e) {
            throw new BusiException(getErrorCode("1000", "15000"), "删除中间表数据失败" + e.getMessage());
        }
